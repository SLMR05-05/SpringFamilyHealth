...
export default function FamilyDashboard() {
  ...
  useEffect(() => {
    const fetchUserHealthData = async () => {
      ...
      try {
        ...
        // Fetch visit history by memberId
        const visitResponse = await visitHistoryApi.getByMemberId(effectiveUserId);
        const rawVisitData = visitResponse?.data || visitResponse?.result || visitResponse || [];
        // Handle paginated response
        let visitData = [];
        if (rawVisitData && typeof rawVisitData === 'object' && !Array.isArray(rawVisitData)) {
          visitData = rawVisitData.content || rawVisitData.items || rawVisitData.data || [];
        } else {
          visitData = Array.isArray(rawVisitData) ? rawVisitData : [];
        }
        // Filter by memberId matching effectiveUserId
        const filteredVisits = visitData.filter(visit => String(visit.memberId) === String(effectiveUserId));
        setVisitHistory(filteredVisits);
  ...
  const stats = {
    totalMembers: dashboardData?.statistics?.totalMembers ?? 0,
    healthyMembers: dashboardData?.statistics?.healthyMembers ?? 0,
    needAttention: dashboardData?.statistics?.needAttention ?? 0,
    upcomingAppointments: dashboardData?.statistics?.upcomingAppointments ?? 0,
    unreadNotifications: dashboardData?.statistics?.unreadNotifications ?? 0
  };
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        {loading ? (
          ...
          <div className="space-y-8">
            ...
            {/* Tab Content */}
            <div>
              {activeTab === 'members' && <MembersView members={familyMembers} familyId={familyId} />}
              {activeTab === 'records' && (
                <RecordsView
